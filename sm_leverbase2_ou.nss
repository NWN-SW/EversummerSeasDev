...
void main()
{
    ...
    object oTest = GetObjectByTag("TEST");
    int nLever1 = GetLocalInt(GetObjectByTag("LEVER_1"), "LEVER_STATE");

    //In the 4 lines of code below, replace "HERE" with your animation string in the correct slot.

    //From deactivated to activated
    ReplaceObjectAnimation(oTest,"off2on","c_buried2dug");

    //Activated
    ReplaceObjectAnimation(oTest,"on","c_dug");

    //From activated to deactivated
    //ReplaceObjectAnimation(oTest,"on2off","on2off");

    //Deactivated
    //ReplaceObjectAnimation(oTest,"off","off");
    ...
    if(nLever1 == 1)
    {
        ...
        AssignCommand(oTest, PlayAnimation(ANIMATION_PLACEABLE_ACTIVATE, 1.0, 11.16));
    }
    ...
    else
    {
        ...
        AssignCommand(oTest, PlayAnimation(ANIMATION_PLACEABLE_DEACTIVATE, 1.0, 11.16));
    }
